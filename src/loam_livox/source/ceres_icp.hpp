...
struct ceres_icp_point2point_mb {
  ...
  _T m_weigh;
  ceres_icp_point2point_mb(
      const Eigen::Matrix<_T, 3, 1> current_pt,
  ...
  bool operator()(const T* _q, const T* _t, T* residual) const {
    Eigen::Quaternion<T> q_last{(T)m_q_last(0), (T)m_q_last(1), (T)m_q_last(2),
                                (T)m_q_last(3)};
    ...
    Eigen::Quaternion<T> q_interpolate =
        Eigen::Quaternion<T>::Identity().slerp((T)m_motion_blur_s, q_incre);
    Eigen::Matrix<T, 3, 1> t_interpolate = t_incre * T(m_motion_blur_s);

    Eigen::Matrix<T, 3, 1> pt{T(m_current_pt(0)), T(m_current_pt(1)),
                              T(m_current_pt(2))};
...
struct ceres_icp_point2line_mb {
  ...
  bool operator()(const T* _q, const T* _t, T* residual) const {
    ...
    Eigen::Matrix<T, 3, 1> residual_vec =
        vec_ac -
        Eigen_math::vector_project_on_unit_vector(vec_ac, vec_line_ab_unit);
  ...
  static ceres::CostFunction* Create(
    ...
    return (new ceres::AutoDiffCostFunction<ceres_icp_point2line_mb, 3, 4, 3>(
        new ceres_icp_point2line_mb(current_pt, target_line_a, target_line_b,
                                    motion_blur_s, q_last, t_last)));
  }
};
...
struct ceres_icp_point2plane_mb {
  ...
  bool operator()(const T* _q, const T* _t, T* residual) const {
    ...
    Eigen::Matrix<T, 3, 1> tar_line_pt_a = m_target_line_a.template cast<T>();
    Eigen::Matrix<T, 3, 1> vec_line_plane_norm =
        m_unit_vec_n.template cast<T>();

    Eigen::Matrix<T, 3, 1> vec_ad = pt_transfromed - tar_line_pt_a;
    Eigen::Matrix<T, 3, 1> residual_vec =
        Eigen_math::vector_project_on_unit_vector(vec_ad, vec_line_plane_norm) *
        T(m_weigh);
    ...
    // cout << " *** " << residual_vec[0] << "  " << residual_vec[1] << "  " <<
    // residual_vec[2] ; cout << " *** " << residual[0] << "  " << residual[1]
    // << "  " << residual[2] << " *** " << endl;
    return true;
  };
  ...
  static ceres::CostFunction* Create(
      ...
      Eigen::Matrix<_T, 3, 1> t_last = Eigen::Matrix<_T, 3, 1>(0, 0, 0)) {
    // TODO: can be vector or distance
    return (new ceres::AutoDiffCostFunction<ceres_icp_point2plane_mb, 3, 4, 3>(
        new ceres_icp_point2plane_mb(current_pt, target_line_a, target_line_b,
                                     target_line_c, motion_blur_s, q_last,
                                     t_last)));
  }
};
...
struct ceres_icp_point2line {
  ...
  static ceres::CostFunction* Create(
    ...
    return (new ceres::AutoDiffCostFunction<ceres_icp_point2line, 3, 4, 3>(
        new ceres_icp_point2line(current_pt, target_line_a, target_line_b,
                                 q_last, t_last)));
  }
};
...
struct ceres_icp_point2plane {
  ...
  static ceres::CostFunction* Create(
    ...
    return (new ceres::AutoDiffCostFunction<ceres_icp_point2plane, 3, 4, 3>(
        new ceres_icp_point2plane(current_pt, target_line_a, target_line_b,
                                  target_line_c, q_last, t_last)));
  }
};
