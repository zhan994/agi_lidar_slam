...
struct Constraint3d {
  ...
  Constraint3d() { information.setIdentity(); };
  Constraint3d(int id_begin, int id_end, Eigen::Quaterniond& q,
               Eigen::Vector3d& p)
      : id_begin(id_begin), id_end(id_end), t_be(q, p) {
...
void pose_graph_optimization_from_g2o_file(std::string& in_file_name) {
  std::cout << "*************************************" << std::endl;
  std::cout << "Post graph optimization form g2o file: " << in_file_name
            << std::endl;
  std::cout << "*************************************" << std::endl;
  MapOfPoses poses;
  VectorOfConstraints constraints;
...
class Mapping_refine {
 public:
  ...
  Mapping_refine() { set_down_sample_resolution(0.2); };
  ...
  static bool load_poses_from_txt_files(
      std::string file_name, Ceres_pose_graph_3d::MapOfPoses& pose3d_map) {
    std::ifstream infile(file_name.c_str());
    if (!infile) {
      std::cout << "Open file: " << file_name << " fail, please check"
                << std::endl;
      return false;
    }
  ...
  static bool load_filename_from_txt_files(
      std::string file_name, std::map<int, std::string>& map_file_name) {
    std::ifstream infile(file_name.c_str());
    if (!infile) {
      std::cout << "Open file: " << file_name << " fail, please check"
                << std::endl;
  ...
  static std::vector<Eigen::Matrix<T, 3, 1>> refine_pts(
      std::vector<Eigen::Matrix<T, 3, 1>>& raw_pt_vec,
      const Eigen::Matrix<TT, 3, 3>& R_mat_ori,
      Eigen::Matrix<TT, 3, 1>& t_vec_ori,
      const Eigen::Matrix<TT, 3, 3>& R_mat_opm,
      Eigen::Matrix<TT, 3, 1>& t_vec_opm) {
    std::vector<Eigen::Matrix<float, 3, 1>> opm_pt_vec;
    opm_pt_vec.resize(raw_pt_vec.size());
    Eigen::Matrix<T, 3, 3> R_aff =
        (R_mat_opm * R_mat_ori.transpose()).template cast<T>();
    Eigen::Matrix<T, 3, 1> T_aff =
        (R_aff.template cast<TT>() * (-t_vec_ori) + t_vec_opm)
            .template cast<T>();
  ...
  pcl::PointCloud<_PointType> refine_pointcloud(
      std::map<int, pcl::PointCloud<_PointType>>& map_idx_pc,
      Ceres_pose_graph_3d::MapOfPoses& pose3d_map_oir,
      Ceres_pose_graph_3d::MapOfPoses& pose3d_map_opm, int idx = 0,
      int if_save = 1) {
  ...
  void refine_mapping(std::map<int, pcl::PointCloud<_PointType>>& map_idx_pc,
    ...
    for (auto it = pose3d_map_oir.begin(); it != pose3d_map_oir.end(); it++) {
      ...
      pts_vec_ori = PCL_TOOLS::pcl_pts_to_eigen_pts<float, _PointType>(
          map_idx_pc.find(id)->second.makeShared());
      pts_vec_opm =
          refine_pts(pts_vec_ori, pose_ori.q.toRotationMatrix(), pose_ori.p,
                     pose_opm.q.toRotationMatrix(), pose_opm.p);
      pcl::PointCloud<_PointType> pcl_pts =
          PCL_TOOLS::eigen_pt_to_pcl_pointcloud<_PointType>(pts_vec_opm);
      if (if_save) {
        pts_vec_opm = PCL_TOOLS::pcl_pts_to_eigen_pts<float, _PointType>(
            pcl_pts.makeShared());
        Points_cloud_map<float> pt_cell_temp;
  ...
  void refine_mapping(std::string& path_name, int if_save = 1) {
    ...
    for (auto it = filename_map.begin(); it != filename_map.end(); it++) {
      ...
      map_idx_pc.insert(std::make_pair(
          id,
          PCL_TOOLS::eigen_pt_to_pcl_pointcloud<pcl::PointXYZ>(pts_vec_ori)));
    }
    refine_mapping<pcl::PointXYZ>(map_idx_pc, pose3d_map_oir, pose3d_map_opm,
                                  if_save);

    m_down_sample_filter.setInputCloud(m_pts_aft_refind.makeShared());
    m_down_sample_filter.filter(m_pts_aft_refind);
    pt_cell_full.set_resolution(10.0);
    pt_cell_full.set_point_cloud(
        PCL_TOOLS::pcl_pts_to_eigen_pts<float, pcl::PointXYZ>(
            m_pts_aft_refind.makeShared()));
