...
class Points_cloud_cell {
 public:
  ...
  Eigen::Matrix<COMP_TYPE, 3, 3> m_cov_mat, m_cov_mat_last,
      m_cov_mat_avoid_singularity;
  Eigen::Matrix<COMP_TYPE, 3, 3> m_icov_mat;

  /** \brief Eigen vectors of voxel covariance matrix */
  Eigen::Matrix<COMP_TYPE, 3, 3>
      m_eigen_vec;  // Eigen vector of covariance matrix
  Eigen::Matrix<COMP_TYPE, 3, 1>
      m_eigen_val;  // Eigen value of covariance values
  ...
  std::shared_ptr<Points_cloud_cell<DATA_TYPE>> m_previous_visited_cell =
      nullptr;
 ...
 public:
  ...
  FUNC_T std::string to_json_string() {
    ...
    // See more details in
    // https://github.com/Tencent/rapidjson/blob/master/example/simplewriter/simplewriter.cpp
  ...
  FUNC_T void save_to_file(const std::string& path = std::string("./"),
    ...
    if (file_name.compare("") == 0) {
      str_ss << path << "/" << std::setprecision(3) << m_center(0) << "_"
             << m_center(1) << "_" << m_center(2) << ".json";
    } else {
    ...
    if (ofs.is_open()) {
      ofs << to_json_string();
      ofs.close();
      std::cout << " Successful. Number of points = " << m_points_vec.size()
                << std::endl;
    } else {
  ...
  FUNC_T int get_points_count() { return m_points_vec.size(); }
  ...
  FUNC_T void covmat_eig_decompose(int if_force_update = 0) {
    if (if_force_update ==
        0)  // if update is not so large, skip eigendecompse to save time.
    {
      if (m_last_eigen_decompose_size > 1000 &&
          (m_last_eigen_decompose_size / m_points_vec.size() > 0.90)) {
        return;
      }
    }
  ...
  FUNC_T Eigen::Matrix<COMP_TYPE, 3, 3>
  get_cov_mat_avoid_singularity()  // Avoids matrices near singularities
                                   // (eq 6.11)[Magnusson 2009]
  {
    ...
    if (!IF_EIGEN_REPLACE) min_covar_eigvalue_mult_ = 0;
    ...
    if (m_covmat_need_update) {
      ...
      if (pt_size < 5 || m_if_incremental_update_mean_and_cov == false) {
        ...
        for (size_t i = 0; i < pt_size; i++) {
          m_cov_mat =
              m_cov_mat + (m_points_vec[i] * m_points_vec[i].transpose())
                              .template cast<COMP_TYPE>();
        }
  ...
  FUNC_T pcl::PointCloud<pcl_pt> get_pointcloud() {
    ...
    if (!IF_ENABLE_DUMP_PCL_PTS) {
      m_pcl_pc_vec = PCL_TOOLS::eigen_pt_to_pcl_pointcloud<pcl_pt, Eigen_Point>(
          m_points_vec);
    }
  ...
  FUNC_T void set_pointcloud(pcl::PointCloud<pcl_pt>& pc_in) {
    ...
    m_pcl_pc_vec = pc_in;
    m_points_vec =
        PCL_TOOLS::pcl_pts_to_eigen_pts<float, pcl_pt>(pc_in.makeShared());
  }
  ...
  FUNC_T void append_pt(const PT_TYPE& pt) {
    ...
    if (m_if_incremental_update_mean_and_cov) {
      ...
      if (N > 5) {
        m_cov_mat =
            ((N - 1) * m_cov_mat_last +
             (P_new - m_mean_last) * ((P_new - m_mean_last).transpose()) +
             (N + 1) * (m_mean_last - m_mean) *
                 ((m_mean_last - m_mean).transpose()) +
  ...
  FUNC_T void set_target_pc(const std::vector<PT_TYPE>& pt_vec) {
    std::unique_lock<std::mutex> lock(*m_mutex_cell);
    // "The three-dimensional normal-distributions transform: an efficient
  ...
  FUNC_T Feature_type determine_feature(int if_recompute = 0) {
    ...
    if ((m_center.template cast<float>() - m_mean.template cast<float>())
            .norm() > m_resolution * 0.75) {
      m_feature_type = e_feature_sphere;
      m_feature_vector << 0, 0, 0;
      return e_feature_sphere;
    }

    if ((m_eigen_val[1] * m_feature_determine_threshold_plane >
         m_eigen_val[0])) {
...
class Points_cloud_map {
 public:
...
#if USE_HASH
  typedef std::unordered_map<PT_TYPE, Mapping_cell_ptr,
                             PCL_TOOLS::Eigen_pt_hasher,
                             PCL_TOOLS::Eigen_pt_compare>
      Map_pt_cell;
  typedef typename std::unordered_map<
      PT_TYPE, Mapping_cell_ptr, PCL_TOOLS::Eigen_pt_hasher,
      PCL_TOOLS::Eigen_pt_compare>::iterator Map_pt_cell_it;
#else
  typedef std::map<PT_TYPE, Mapping_cell_ptr, PCL_TOOLS::Pt_compare>
      Map_pt_cell;
  typedef typename std::map<PT_TYPE, Mapping_cell_ptr,
                            PCL_TOOLS::Pt_compare>::iterator Map_pt_cell_it;
#endif
  DATA_TYPE m_x_min, m_x_max;
  DATA_TYPE m_y_min, m_y_max;
  DATA_TYPE m_z_min, m_z_max;
  DATA_TYPE
      m_resolution;  // resolution mean the distance of a cute to its bound.
  Common_tools::Timer m_timer;
  std::vector<Mapping_cell_ptr> m_cell_vec;
  int m_if_incremental_update_mean_and_cov =
      IF_ENABLE_INCREMENTAL_UPDATE_MEAN_COV;
  ...
  FUNC_T int get_cells_size() { return m_map_pt_cell.size(); }
  ...
  FUNC_T PT_TYPE find_cell_center(const PT_TYPE& pt) {
    ...
    // cell_center( 0 ) = ( std::round( ( pt( 0 ) - m_x_min - half_of_box_size )
    // / box_size ) ) * box_size + m_x_min + half_of_box_size; cell_center( 1 )
    // = ( std::round( ( pt( 1 ) - m_y_min - half_of_box_size ) / box_size ) ) *
    // box_size + m_y_min + half_of_box_size; cell_center( 2 ) = ( std::round( (
    // pt( 2 ) - m_z_min - half_of_box_size ) / box_size ) ) * box_size +
    // m_z_min + half_of_box_size;

    cell_center(0) =
        (std::round((pt(0) - half_of_box_size) / box_size)) * box_size +
        half_of_box_size;
    cell_center(1) =
        (std::round((pt(1) - half_of_box_size) / box_size)) * box_size +
        half_of_box_size;
    cell_center(2) =
        (std::round((pt(2) - half_of_box_size) / box_size)) * box_size +
        half_of_box_size;
  ...
  FUNC_T void set_point_cloud(
      const std::vector<PT_TYPE>& input_pt_vec,
      std::set<std::shared_ptr<Mapping_cell>>* cell_vec = nullptr) {
    clear_data();
    ...
    screen_out << "*** set_point_cloud octree initialization finish ***"
               << std::endl;
    m_initialized = true;
    m_current_frame_idx++;
  }

  FUNC_T void append_cloud(const std::vector<PT_TYPE>& input_pt_vec,
                           std::set<Mapping_cell_ptr>* cell_vec = nullptr) {
    ...
    if (current_size == 0) {
      ...
      if (cell_vec != nullptr) {
        for (auto it = appeared_cell_count.begin();
             it != appeared_cell_count.end(); it++) {
          if (it->second >= 3) {
  ...
  FUNC_T DATA_TYPE get_resolution() { return m_resolution * 2; }
  ...
  FUNC_T Mapping_cell_ptr add_cell(const PT_TYPE& cell_center) {
    ...
    Mapping_cell_ptr cell =
        std::make_shared<Mapping_cell>(cell_center, (DATA_TYPE)m_resolution);
    cell->m_create_frame_idx = m_current_frame_idx;
    cell->m_last_update_frame_idx = m_current_frame_idx;
    cell->m_if_incremental_update_mean_and_cov =
        m_if_incremental_update_mean_and_cov;
    m_map_pt_cell.insert(std::make_pair(cell_center, cell));

    if (m_initialized == false) {
      m_pcl_cells_center->push_back(pcl::PointXYZ(
          cell->m_center(0), cell->m_center(1), cell->m_center(2)));
    } else {
      std::unique_lock<std::mutex> lock(*m_octotree_mutex);
      m_octree.addPointToCloud(
          pcl::PointXYZ(cell->m_center(0), cell->m_center(1),
                        cell->m_center(2)),
          m_pcl_cells_center);
    }
  ...
  FUNC_T Mapping_cell_ptr find_cell(const PT_TYPE& pt, int if_add = 1,
    ...
    if (it == m_map_pt_cell.end()) {
      ...
      if (if_treat_revisit) {
        if (m_current_frame_idx - it->second->m_last_update_frame_idx <
            m_minimum_revisit_threshold) {
          it->second->m_last_update_frame_idx = m_current_frame_idx;
          return it->second;
        } else {
          // Avoid confilcts of revisited
          // ENABLE_SCREEN_PRINTF;
          // screen_out << "!!!!! Cell revisit, curr_idx = " <<
          // m_current_frame_idx << " ,last_idx = " <<
          // it->second->m_last_update_frame_idx << std::endl;

          Mapping_cell_ptr new_cell = std::make_shared<Mapping_cell>(
              it->second->get_center(), (DATA_TYPE)m_resolution);
          m_cell_vec.push_back(new_cell);
          new_cell->m_previous_visited_cell = it->second;
          it->second = new_cell;
          it->second->m_create_frame_idx = m_current_frame_idx;
          it->second->m_last_update_frame_idx = m_current_frame_idx;
          // screen_out << ", find cell addr = " << ( void * ) find_cell( pt )
          // << std::endl;
        }
      }
      return it->second;
  ...
  template <typename T>
  FUNC_T std::vector<Mapping_cell_ptr> find_cells_in_radius(
      T pt, float searchRadius = 0) {
    std::unique_lock<std::mutex> lock(*m_octotree_mutex);
    ...
    if (searchRadius == 0) {
      m_octree.radiusSearch(searchPoint, m_resolution, cloudNWRSearch,
                            cloudNWRRadius);
    } else {
      m_octree.radiusSearch(searchPoint, searchRadius, cloudNWRSearch,
                            cloudNWRRadius);
    }

    PT_TYPE eigen_pt;
    for (size_t i = 0; i < cloudNWRSearch.size(); i++) {
      eigen_pt = PCL_TOOLS::pcl_pt_to_eigen<DATA_TYPE>(
          m_octree.getInputCloud()->points[cloudNWRSearch[i]]);
  ...
  FUNC_T std::string to_json_string(int& avail_cell_size = 0) {
    ...
    str_s << "[";
    avail_cell_size = 0;
    for (Map_pt_cell_it it = m_map_pt_cell.begin();
         it != m_map_pt_cell.end();) {
  ...
  FUNC_T void save_to_file(const std::string& path = std::string("./"),
    ...
    if (ofs.is_open()) {
      ...
      ofs.close();
      screen_out << " Successful. Number of cell = " << avail_cell_size
                 << std::endl;
    } else {
  ...
  FUNC_T int load_mapping_from_file(
      const std::string& file_name = std::string("./mapping.json")) {
    Common_tools::Timer timer;
    timer.tic("Load mapping from json file");
    FILE* fp = fopen(file_name.c_str(), "r");
    if (fp == nullptr) {
      std::cout << "load_mapping_from_file: " << file_name << " fail!"
                << std::endl;
      return 0;
    } else {
      ...
      for (unsigned int i = 0; i < doc.Size(); ++i) {
        ...
        Mapping_cell* cell = add_cell(Eigen::Matrix<DATA_TYPE, 3, 1>(
            get_json_array<DATA_TYPE>(doc[i]["Center"].GetArray())));

        cell->m_mean = Eigen::Matrix<COMP_TYPE, 3, 1>(
            get_json_array<COMP_TYPE>(doc[i]["Mean"].GetArray()));
        cell->m_cov_mat = Eigen::Matrix<COMP_TYPE, 3, 3>(
            get_json_array<COMP_TYPE>(doc[i]["Cov"].GetArray()));
        cell->m_icov_mat = Eigen::Matrix<COMP_TYPE, 3, 3>(
            get_json_array<COMP_TYPE>(doc[i]["Icov"].GetArray()));
        cell->m_eigen_vec = Eigen::Matrix<COMP_TYPE, 3, 3>(
            get_json_array<COMP_TYPE>(doc[i]["Eig_vec"].GetArray()));
        cell->m_eigen_val = Eigen::Matrix<COMP_TYPE, 3, 1>(
            get_json_array<COMP_TYPE>(doc[i]["Eig_val"].GetArray()));
        ...
        for (size_t pt_idx = 0; pt_idx < pt_num; pt_idx++) {
          cell->m_points_vec[pt_idx] << pt_vec_data[pt_idx * 3 + 0],
              pt_vec_data[pt_idx * 3 + 1], pt_vec_data[pt_idx * 3 + 2];
          cell->m_xyz_sum =
              cell->m_xyz_sum +
              cell->m_points_vec[pt_idx].template cast<COMP_TYPE>();
        }
        delete pt_vec_data;
      }
      fclose(fp);
  ...
  FUNC_T std::vector<Eigen::Matrix<DATA_TYPE, 3, 1>> load_pts_from_file(
      const std::string& file_name = std::string("./mapping.json")) {
    Common_tools::Timer timer;
    ...
    if (fp == nullptr) {
      ...
      for (unsigned int i = 0; i < doc.Size(); ++i) {
        ...
        pt_vec_data = get_json_array<DATA_TYPE>(doc[i]["Pt_vec"].GetArray());
        for (size_t pt_idx = 0; pt_idx < pt_num; pt_idx++) {
          pt_vec_cell[pt_idx] << pt_vec_data[pt_idx * 3 + 0],
              pt_vec_data[pt_idx * 3 + 1], pt_vec_data[pt_idx * 3 + 2];
        }
        res_vec.insert(res_vec.end(), pt_vec_cell.begin(), pt_vec_cell.end());
      }
      fclose(fp);
      std::cout << "****** Load point from:" << file_name
                << "  successful ****** " << std::endl;
  ...
  // template <typename T>
  FUNC_T std::vector<PT_TYPE> query_point_cloud(
  ...
  FUNC_T pcl::PointCloud<pcl_pt> extract_specify_points(
      Feature_type select_type) {
...
class Maps_keyframe {
 public:
  ...
  Eigen::Map<Eigen::Quaterniond> m_pose_q =
      Eigen::Map<Eigen::Quaterniond>(m_pose_buffer);
  Eigen::Map<Eigen::Vector3d> m_pose_t =
      Eigen::Map<Eigen::Vector3d>(m_pose_buffer + 4);
  ...
  int scale = 10;
  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> m_feature_img_line,
      m_feature_img_plane;
  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> m_feature_img_line_roi,
      m_feature_img_plane_roi;

  std::vector<Eigen::Matrix<DATA_TYPE, 3, 1>> m_feature_vecs_plane,
      m_feature_vecs_line;
  std::vector<Eigen::Matrix<DATA_TYPE, 3, 1>> m_feature_vecs_plane_roi,
      m_feature_vecs_line_roi;
  ...
  pcl::PointCloud<PointType>
      m_accumulated_point_cloud;  // The pointcloud sampled from current frame
                                  // to last frame
  ...
  Maps_keyframe() {
    ...
    m_accumulated_point_cloud.reserve(5000 * 500 /
                                      3);  // each frame have 5000/3 pts, about
                                           // maximum 500 frame of accumulation.
  };
  ...
  template <typename T>
  FUNC_T void feature_direction(Eigen::Matrix<T, 3, 1>& vec_3d, int& phi_idx,
                                int& theta_idx) {
    if (vec_3d[0] < 0) {
      vec_3d *= (-1.0);
    }
  ...
  template <typename T>
  FUNC_T static void refine_feature_img(
      Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& feature_img) {
    ...
    if ((feature_img.block(0, 0, 2, round(cols / 2))).maxCoeff() <
        (feature_img.block(0, round(cols / 2), 2, round(cols / 2)))
            .maxCoeff()) {
      feature_img = feature_img.rowwise().reverse().eval();
    }
  }

  FUNC_T static float ratio_of_nonzero_in_img(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img) {
    int count = 0;
    for (int i = 0; i < img.rows(); i++) {
      for (int j = 0; j < img.cols(); j++)
        if (img(i, j) >= 1.0) count++;
    }
    return (float)(count) / (img.rows() * img.cols());
  }

  // ANCHOR keyframe::max_similiarity_of_two_image
  FUNC_T static float max_similiarity_of_two_image(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_a,
    ...
    if (0) {
      for (size_t i = 0; i < rows * 1.0; i++) {
        // Eigen::Matrix< float, Eigen::Dynamic, Eigen::Dynamic > img_b_roi =
        // img_b.block( i, 0, ( int ) std::round( rows / 2 ), cols );
        Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> img_b_roi_up =
            img_b.block(i, 0, rows - i, cols);
        Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> img_b_roi_down =
            img_b.block(0, 0, i, cols);

        img_b_roi << img_b_roi_up, img_b_roi_down;

        res = similiarity_of_two_image_opencv(img_a, img_b_roi);

        if (fabs(res) > fabs(max_res)) max_res = fabs(res);
      }
    } else {
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> res_img;
      res_img = add_padding_to_feature_image(img_b, PHI_RESOLUTION * 0.5,
  ...
  FUNC_T static float similiarity_of_two_image_opencv(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_a,
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_b,
    ...
    return cv::compareHist(
        hist_a, hist_b,
        method);  // https://docs.opencv.org/2.4/modules/imgproc/doc/histograms.html?highlight=comparehist
  }

  FUNC_T static float max_similiarity_of_two_image_opencv(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_a,
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_b,
      int method = CV_COMP_CORREL) {
    cv::Mat hist_a, hist_b;
    int cols = img_a.cols();
    int rows = img_a.rows();
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> img_a_roi =
        img_a.block(0, 0, rows, cols);
    ...
    return maxVal;
    // return cv::compareHist( hist_a, hist_b, method ); //
  ...
  FUNC_T void add_cells(const std::set<Mapping_cell_ptr>& cells_vec) {
    ...
    for (typename std::set<Mapping_cell_ptr>::iterator it = cells_vec.begin();
         it != cells_vec.end(); it++) {
      ...
      if (last_cell_numbers != m_set_cell.size())  // New different cell
      {
        if (last_cell_numbers == 0) {
          // Init octree
          m_pcl_cells_center->push_back(pcl::PointXYZ(
              (*it)->m_center(0), (*it)->m_center(1), (*it)->m_center(2)));
        }
  ...
  FUNC_T pcl::PointCloud<pcl_pt> extract_specify_points(
    ...
    // screen_out << "Type is " << ( int ) ( select_type ) << ", total is "<<
    // m_set_cell.size()
    //            << ", cell of type: " << cell_is_selected_type << " , size of
    //            pts is " << res_pt.points.size() << std::endl;
  ...
  FUNC_T float get_ratio_range_of_cell(
      PT_TYPE& cell_center = PT_TYPE(0, 0, 0), float ratio = 0.8,
  ...
  add_padding_to_feature_image(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& in_mat,
      int padding_size_x, int padding_size_y) {
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> res_img;
    ...
    // Top left
    res_img.block(0, 0, padding_size_y, padding_size_x) =
        in_mat.block(size_h - padding_size_y, size_w - padding_size_x,
                     padding_size_y, padding_size_x);
    // Top right
    res_img.block(0, size_w + padding_size_x, padding_size_y, padding_size_x) =
        in_mat.block(size_h - padding_size_y, 0, padding_size_y,
                     padding_size_x);
    // Bottom left
    res_img.block(size_h + padding_size_y, 0, padding_size_y, padding_size_x) =
        in_mat.block(0, size_w - padding_size_x, padding_size_y,
                     padding_size_x);
    // Bottom right
    res_img.block(size_h + padding_size_y, size_w + padding_size_x,
                  padding_size_y, padding_size_x) =
        in_mat.block(0, 0, padding_size_y, padding_size_x);

    // Padding four blocks
    // Up
    res_img.block(0, padding_size_x, padding_size_y, size_w) =
        in_mat.block(size_h - padding_size_y, 0, padding_size_y, size_w);
    // Down
    res_img.block(size_h + padding_size_y, padding_size_x, padding_size_y,
                  size_w) = in_mat.block(0, 0, padding_size_y, size_w);
    // Left
    res_img.block(padding_size_y, 0, size_h, padding_size_x) =
        in_mat.block(0, size_w - padding_size_x, size_h, padding_size_x);
    // Right
    res_img.block(padding_size_y, size_w + padding_size_x, size_h,
                  padding_size_x) = in_mat.block(0, 0, size_h, padding_size_x);

    return res_img;
  }

  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> apply_guassian_blur(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& in_mat,
    ...
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> res_img;
    res_img =
        add_padding_to_feature_image(in_mat, kernel_size + 1, kernel_size + 1);
    cv::eigen2cv(res_img, cv_img);
    cv::GaussianBlur(cv_img, cv_img, kernel, sigma);
    cv::cv2eigen(cv_img, res_img);
    return res_img.block(kernel_size + 1, kernel_size + 1, in_mat.rows(),
                         in_mat.cols());
  }
  ...
  FUNC_T void generate_feature_img(
      std::vector<Eigen::Matrix<DATA_TYPE, 3, 1>>& feature_vecs_line,
    ...
    eigen_decompose_of_featurevector(feature_vecs_plane, eigen_vector,
                                     eigen_val);
    ...
    for (size_t i = 0; i < feature_vecs_plane.size(); i++) {
      ...
      feature_img_plane(theta_idx, beta_idx) =
    ...
    for (size_t i = 0; i < feature_vecs_line.size(); i++) {
      ...
      feature_direction(affined_vector, theta_idx, beta_idx);
      feature_img_line(theta_idx, beta_idx) =
  ...
  FUNC_T void extract_feature_mapping_new(
      std::set<Mapping_cell_ptr> cell_vec,
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& feature_img_line,
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& feature_img_plane,
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>&
          feature_img_line_roi,
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>&
          feature_img_plane_roi,
      int if_recompute = 0) {
    ...
    for (auto it = cell_vec.begin(); it != cell_vec.end(); it++) {
      ...
      if (feature_type == Feature_type::e_feature_line) {
        m_feature_vecs_line.push_back(
            cell->m_feature_vector.template cast<DATA_TYPE>());
      }
      if (feature_type == Feature_type::e_feature_plane) {
        m_feature_vecs_plane.push_back(
            cell->m_feature_vector.template cast<DATA_TYPE>());
      }
      if ((cell->get_center() - keyframe_center).norm() < m_roi_range) {
        if (feature_type == Feature_type::e_feature_line) {
          m_feature_vecs_line_roi.push_back(
              cell->m_feature_vector.template cast<DATA_TYPE>());
        }
        if (feature_type == Feature_type::e_feature_plane) {
          m_feature_vecs_plane_roi.push_back(
              cell->m_feature_vector.template cast<DATA_TYPE>());
        }
      }
    }
    ...
    screen_out << ", plane_cell_all: " << m_feature_vecs_plane.size()
               << std::endl;

    generate_feature_img(m_feature_vecs_line_roi, m_feature_vecs_plane_roi,
                         feature_img_line_roi, feature_img_plane_roi);
    generate_feature_img(m_feature_vecs_line, m_feature_vecs_plane,
                         feature_img_line, feature_img_plane);
  };
  ...
  FUNC_T int analyze(int if_recompute = 0) {
    ...
    extract_feature_mapping_new(m_set_cell, m_feature_img_line,
                                m_feature_img_plane, m_feature_img_line_roi,
                                m_feature_img_plane_roi, if_recompute);
    return 0;
  }
  ...
  template <typename T>
  FUNC_T void eigen_decompose_of_featurevector(
      std::vector<Eigen::Matrix<T, 3, 1>>& feature_vectors,
    ...
    for (size_t i = 0; i < feature_vectors.size(); i++) {
      mat_cov = mat_cov + (feature_vectors[i] * feature_vectors[i].transpose())
  ...
  FUNC_T static float similiarity_of_two_image(
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_a,
      const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>& img_b) {
  ...
  void save_feature_image_to_json(std::string file_name) {
    ...
    if (!ofs.is_open()) {
      screen_out << "Open file " << file_name
                 << " fail!!! please check: " << std::endl;
    }
    rapidjson::Document document;
    rapidjson::StringBuffer sb;
#if IF_JSON_PRETTY_WRITTER
    ...
    Common_tools::save_mat_to_jason_writter(writer, "Plane",
                                            m_feature_img_plane);
    Common_tools::save_mat_to_jason_writter(writer, "Line_roi",
                                            m_feature_img_line_roi);
    Common_tools::save_mat_to_jason_writter(writer, "Plane_roi",
                                            m_feature_img_plane_roi);
    writer.EndObject();
  ...
  void display() {
    ...
    for (auto it = m_set_cell.begin(); it != m_set_cell.end(); it++) {
      screen_out << "Center of cell is: " << (*(it))->get_center().transpose()
                 << std::endl;
    }
    m_if_verbose_screen_printf = 1;
  }
