...
class Point_cloud_registration {
 public:
  ...
  ceres::LinearSolverType slover_type =
      ceres::DENSE_SCHUR;  // SPARSE_NORMAL_CHOLESKY | DENSE_QR | DENSE_SCHUR
  ...
  double compute_inlier_residual_threshold(std::vector<double> residuals,
                                           double ratio) {
  ...
  int find_out_incremental_transfrom(
      pcl::PointCloud<PointType>::Ptr in_laser_cloud_corner_from_map,
    ...
    Eigen::Map<Eigen::Quaterniond> q_w_incre =
        Eigen::Map<Eigen::Quaterniond>(m_para_buffer_incremental);
    Eigen::Map<Eigen::Vector3d> t_w_incre =
        Eigen::Map<Eigen::Vector3d>(m_para_buffer_incremental + 4);

    m_kdtree_corner_from_map = kdtree_corner_from_map;
    m_kdtree_surf_from_map = kdtree_surf_from_map;

    pcl::PointCloud<PointType> laser_cloud_corner_from_map =
        *in_laser_cloud_corner_from_map;
    pcl::PointCloud<PointType> laser_cloud_surf_from_map =
    ...
    *(m_logger_timer->get_ostream())
        << m_timer->toc_string("Query points for match") << std::endl;
    ...
    if (laserCloudCornerFromMapNum > CORNER_MIN_MAP_NUM &&
      ...
      *(m_logger_timer->get_ostream())
          << m_timer->toc_string("Build kdtree") << std::endl;
      ...
      for (iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++) {
        ...
        for (int i = 0; i < laser_corner_pt_num; i++) {
          if (laser_corner_pt_num > 2 * m_maximum_allow_residual_block) {
            if (m_rand_float.rand_uniform() * laser_corner_pt_num >
                2 * m_maximum_allow_residual_block) {
              continue;
            }
          }
          ...
          // printf_line;
          if ((!std::isfinite(pointOri.x)) || (!std::isfinite(pointOri.y)) ||
          ...
          if (m_kdtree_corner_from_map.nearestKSearch(
                  pointSel, line_search_num, m_point_search_Idx,
                  m_point_search_sq_dis) != line_search_num) {
            continue;
          }

          if (m_point_search_sq_dis[line_search_num - 1] <
              m_maximum_dis_line_for_match) {
            ...
            if (IF_LINE_FEATURE_CHECK) {
              for (int j = 0; j < line_search_num; j++) {
                Eigen::Vector3d tmp(
                    laser_cloud_corner_from_map.points[m_point_search_Idx[j]].x,
                    laser_cloud_corner_from_map.points[m_point_search_Idx[j]].y,
                    laser_cloud_corner_from_map.points[m_point_search_Idx[j]]
                        .z);
                center = center + tmp;
            ...
            if (line_is_avail) {
              if (ICP_LINE) {
                ...
                auto pt_1 = pcl_pt_to_eigend(
                    laser_cloud_corner_from_map.points[m_point_search_Idx[0]]);
                auto pt_2 = pcl_pt_to_eigend(
                    laser_cloud_corner_from_map.points[m_point_search_Idx[1]]);
                if ((pt_1 - pt_2).norm() < 0.0001) continue;
                if (m_if_motion_deblur) {
                  // printf_line;
                  cost_function = ceres_icp_point2line_mb<double>::Create(
                      curr_point, pt_1, pt_2,
                      refine_blur(pointOri.intensity, m_minimum_pt_time_stamp,
                                  m_maximum_pt_time_stamp) *
                          1.0,
                      Eigen::Matrix<double, 4, 1>(
                          m_q_w_last.w(), m_q_w_last.x(), m_q_w_last.y(),
                          m_q_w_last.z()),
        ...
        for (int i = 0; i < laser_surface_pt_num; i++) {
          if (laser_surface_pt_num > 2 * m_maximum_allow_residual_block) {
            if (m_rand_float.rand_uniform() * laser_surface_pt_num >
                2 * m_maximum_allow_residual_block) {
              continue;
            }
          }
          ...
          if (m_point_search_sq_dis[plane_search_num - 1] <
              m_maximum_dis_plane_for_match) {
            std::vector<Eigen::Vector3d> nearCorners;
            Eigen::Vector3d center(0, 0, 0);
              ...
              if (ICP_PLANE) {
                ...
                if (m_if_motion_deblur) {
                  cost_function = ceres_icp_point2plane_mb<double>::Create(
                      curr_point,
                      pcl_pt_to_eigend(laser_cloud_surf_from_map
                                           .points[m_point_search_Idx[0]]),
                      pcl_pt_to_eigend(
                          laser_cloud_surf_from_map.points
                              [m_point_search_Idx[plane_search_num / 2]]),
                      pcl_pt_to_eigend(
                          laser_cloud_surf_from_map.points
                              [m_point_search_Idx[plane_search_num - 1]]),
                      refine_blur(pointOri.intensity, m_minimum_pt_time_stamp,
                                  m_maximum_pt_time_stamp) *
                          1.0,
                      Eigen::Matrix<double, 4, 1>(
        ...
        if (residual_block_ids.size() >
            (size_t)m_maximum_allow_residual_block) {
          residual_block_ids_temp.clear();

          float threshold_to_reserve = (float)m_maximum_allow_residual_block /
                                       (float)residual_block_ids.size();
          float* probability_to_drop = m_rand_float.rand_array_uniform(
              0, 1.0, residual_block_ids.size());
          screen_out << "Number of residual blocks too Large, drop them to "
                     << m_maximum_allow_residual_block << endl;
        ...
        // If the number of residual block too Large, randomly drop some of them
        // to guarentee the real-time perfromance.
        for (size_t ii = 0; ii < 1; ii++) {
          ...
          double m_inliner_ratio_threshold =
              compute_inlier_residual_threshold(residuals, m_inlier_ratio);
          m_inlier_threshold =
              std::max(m_inliner_dis, m_inliner_ratio_threshold);
          // screen_out << "Inlier threshold is: " << m_inlier_final_threshold
          // << endl;
          for (unsigned int i = 0; i < residual_block_ids.size(); i++) {
            if ((fabs(residuals[3 * i + 0]) + fabs(residuals[3 * i + 1]) +
                 fabs(residuals[3 * i + 2])) >
                m_inlier_threshold)  // std::min( 1.0, 10 * avr_cost )
            {
              // screen_out << "Remove outliers, drop id = " << (void
              // *)residual_block_ids[ i ] <<endl;
              problem.RemoveResidualBlock(residual_block_ids[i]);
            } else {
              residual_block_ids_temp.push_back(residual_block_ids[i]);
            }
        ...
        ceres::Solve(options, &problem, &summary);
        if (m_if_motion_deblur) {
          compute_interpolatation_rodrigue(q_w_incre, m_interpolatation_omega,
                                           m_interpolatation_theta,
                                           m_interpolatation_omega_hat);
          m_interpolatation_omega_hat_sq2 =
        ...
        if (q_last_optimize.angularDistance(q_w_incre) <
                57.3 * m_minimum_icp_R_diff &&
            (t_last_optimize - t_w_incre).norm() < m_minimum_icp_T_diff) {
          screen_out << "----- Terminate, iteration times  = " << iterCount
                     << "-----" << endl;
          break;
        } else {
      ...
      if (laser_corner_pt_num != 0 && laser_surface_pt_num != 0) {
        m_logger_common->printf(
            "Corner  total num %d |  use %d | rate = %d \% \r\n",
            laser_corner_pt_num, corner_avail_num,
            (corner_avail_num)*100 / laser_corner_pt_num);
        m_logger_common->printf(
            "Surface total num %d |  use %d | rate = %d \% \r\n",
            laser_surface_pt_num, surf_avail_num,
            (surf_avail_num)*100 / laser_surface_pt_num);
      }
      *(m_logger_timer->get_ostream())
          << m_timer->toc_string("Pose optimization") << std::endl;
      ...
      *(m_logger_common->get_ostream())
          << "Last R:"
          << m_q_w_last.toRotationMatrix().eulerAngles(0, 1, 2).transpose() *
                 57.3
          << " ,T = " << m_t_w_last.transpose() << endl;
      *(m_logger_common->get_ostream())
          << "Curr R:"
          << m_q_w_curr.toRotationMatrix().eulerAngles(0, 1, 2).transpose() *
                 57.3
          << " ,T = " << m_t_w_curr.transpose() << endl;
      *(m_logger_common->get_ostream())
          << "Iteration time: " << iterCount << endl;

      m_logger_common->printf("Motion blur = %d | ", m_if_motion_deblur);
      m_logger_common->printf(
          "Cost = %.5f| inlier_thr = %.2f |blk_size = %d | corner_num = %d | "
          "surf_num = %d | angle dis = %.2f | T dis = %.2f \r\n",
          minimize_cost, m_inlier_threshold, summary.num_residual_blocks,
          corner_avail_num, surf_avail_num, m_angular_diff, m_t_diff);

      m_inlier_threshold =
          m_inlier_threshold * summary.final_cost / summary.initial_cost;  //

      if (m_angular_diff > m_para_max_angular_rate ||
  ...
  int find_out_incremental_transfrom(
    ...
    if (laser_cloud_corner_from_map.points.size() &&
      ...
      m_kdtree_corner_from_map.setInputCloud(
          laser_cloud_corner_from_map.makeShared());
      m_kdtree_surf_from_map.setInputCloud(
          laser_cloud_surf_from_map.makeShared());
    } else {
      return 1;
    }
    return find_out_incremental_transfrom(
        in_laser_cloud_corner_from_map, in_laser_cloud_surf_from_map,
        m_kdtree_corner_from_map, m_kdtree_surf_from_map, laserCloudCornerStack,
        laserCloudSurfStack);
  }
