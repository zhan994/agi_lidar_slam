...
class Scene_alignment {
 public:
  ...
  pcl::VoxelGrid<PointType> m_down_sample_filter_line_source,
      m_down_sample_filter_line_target;
  pcl::VoxelGrid<PointType> m_down_sample_filter_surface_source,
      m_down_sample_filter_surface_target;
  int pair_idx = 0;
  ...
  Scene_alignment(std::string path) { init(path); };

  ~Scene_alignment(){};

  ADD_SCREEN_PRINTF_OUT_METHOD;

  static int load_pose_and_regerror(
      std::string file_name, Eigen::Quaterniond& q_curr,
    ...
    if (fp == nullptr) {
      ...
      auto json_arrray =
          Common_tools::get_json_array<double>(doc["Q"].GetArray());
  ...
  static void save_edge_and_vertex_to_g2o(
      std::string file_name, Ceres_pose_graph_3d::MapOfPoses& pose3d_map,
    ...
    if (fp != NULL) {
      ...
      for (auto it = pose3d_map.begin(); it != pose3d_map.end(); it++) {
        ...
        fprintf(fp, "VERTEX_SE3:QUAT %d %f %f %f %f %f %f %f\n", (int)it->first,
                pose3d.p(0), pose3d.p(1), pose3d.p(2), pose3d.q.x(),
                pose3d.q.y(), pose3d.q.z(), pose3d.q.w());
      }
      for (size_t i = 0; i < pose_csn_vec.size(); i++) {
        auto csn = pose_csn_vec[i];
        fprintf(fp, "EDGE_SE3:QUAT %d %d %f %f %f %f %f %f %f", csn.id_begin,
                csn.id_end, csn.t_be.p(0), csn.t_be.p(1), csn.t_be.p(2),
  ...
  static void save_edge_and_vertex_to_g2o(
      std::string file_name, Ceres_pose_graph_3d::VectorOfPose pose3d_vec,
      Ceres_pose_graph_3d::VectorOfConstraints& pose_csn_vec) {
    FILE* fp = fopen(file_name.c_str(), "w+");
    if (fp != NULL) {
      cout << "Dump to g2o files:" << file_name << std::endl;
      for (size_t i = 0; i < pose3d_vec.size(); i++) {
        fprintf(fp, "VERTEX_SE3:QUAT %d %f %f %f %f %f %f %f\n", (int)i,
                ...
                pose3d_vec[i].q.x(), pose3d_vec[i].q.y(), pose3d_vec[i].q.z(),
                pose3d_vec[i].q.w());
      }
      for (size_t i = 0; i < pose_csn_vec.size(); i++) {
        auto csn = pose_csn_vec[i];
  ...
  void set_downsample_resolution(const float& line_res,
                                 const float& plane_res) {
    m_line_res = line_res;
    m_plane_res = plane_res;
    m_down_sample_filter_line_source.setLeafSize(m_line_res, m_line_res,
                                                 m_line_res);
    m_down_sample_filter_surface_source.setLeafSize(m_plane_res, m_plane_res,
                                                    m_plane_res);
    m_down_sample_filter_line_target.setLeafSize(m_line_res, m_line_res,
                                                 m_line_res);
    m_down_sample_filter_surface_target.setLeafSize(m_plane_res, m_plane_res,
                                                    m_plane_res);
  }
  ...
  int find_tranfrom_of_two_mappings(
      std::shared_ptr<Maps_keyframe<PT_DATA_TYPE> > keyframe_a,
      std::shared_ptr<Maps_keyframe<PT_DATA_TYPE> > keyframe_b, int if_save = 1,
      std::string mapping_save_path = std::string(" ")) {
    return find_tranfrom_of_two_mappings(keyframe_a.get(), keyframe_b.get(),
                                         if_save, mapping_save_path);
  }

  int find_tranfrom_of_two_mappings(
      Maps_keyframe<PT_DATA_TYPE>* keyframe_a,
      Maps_keyframe<PT_DATA_TYPE>* keyframe_b, int if_save = 1,
      std::string mapping_save_path = std::string(" ")) {
    pcl::PointCloud<pcl_pt> sourece_pt_line =
        keyframe_a->extract_specify_points(Feature_type::e_feature_line);
    pcl::PointCloud<pcl_pt> sourece_pt_plane =
        keyframe_a->extract_specify_points(Feature_type::e_feature_plane);
    pcl::PointCloud<pcl_pt> all_pt_a = keyframe_a->get_all_pointcloud();

    pcl::PointCloud<pcl_pt> target_pt_line =
        keyframe_b->extract_specify_points(Feature_type::e_feature_line);
    pcl::PointCloud<pcl_pt> target_pt_plane =
        keyframe_b->extract_specify_points(Feature_type::e_feature_plane);
    pcl::PointCloud<pcl_pt> all_pt_b = keyframe_b->get_all_pointcloud();

    pcl::PointCloud<pcl_pt> sourece_pt_line_ds,
        sourece_pt_plane_ds;  // Point cloud of downsampled
    pcl::PointCloud<pcl_pt> target_pt_line_ds, target_pt_plane_ds;

    m_down_sample_filter_line_source.setInputCloud(
        sourece_pt_line.makeShared());
    m_down_sample_filter_surface_source.setInputCloud(
        sourece_pt_plane.makeShared());
    m_down_sample_filter_line_target.setInputCloud(target_pt_line.makeShared());
    m_down_sample_filter_surface_target.setInputCloud(
        target_pt_plane.makeShared());
    ...
    m_pc_reg.m_maximum_allow_residual_block =
    ...
    Eigen::Matrix<double, 3, 1> transform_T =
        (keyframe_a->get_center() - keyframe_b->get_center())
    ...
    for (int scale = 8; scale >= 0; scale -= 4) {
      ...
      m_down_sample_filter_line_source.setLeafSize(line_res, line_res,
                                                   line_res);
      m_down_sample_filter_surface_source.setLeafSize(plane_res, plane_res,
                                                      plane_res);
      m_down_sample_filter_line_target.setLeafSize(line_res, line_res,
                                                   line_res);
      m_down_sample_filter_surface_target.setLeafSize(plane_res, plane_res,
                                                      plane_res);
      ...
      screen_out << "Source pt line size = " << sourece_pt_line_ds.size()
                 << " , plane size = " << sourece_pt_plane_ds.size()
                 << std::endl;
      screen_out << "Target pt line size = " << target_pt_line_ds.size()
                 << " , plane size = " << target_pt_plane_ds.size()
                 << std::endl;

      m_pc_reg.find_out_incremental_transfrom(
          sourece_pt_line_ds.makeShared(), sourece_pt_plane_ds.makeShared(),
          target_pt_line_ds.makeShared(), target_pt_plane_ds.makeShared());

      screen_out << "===*** Result of pc_reg: ===*** " << endl;
      screen_out << "Resoulation = " << line_res << " -- " << plane_res << endl;
      screen_out << "Q_incre is: " << m_pc_reg.m_q_w_incre.coeffs().transpose()
                 << endl;
      screen_out << "T_incre is: " << m_pc_reg.m_t_w_incre.transpose() << endl;
      screen_out << "Q_curr is: " << m_pc_reg.m_q_w_curr.coeffs().transpose()
                 << endl;
    ...
    if (if_save) {
      ...
      auto all_pt_temp = pointcloud_transfrom<double, pcl_pt>(
          all_pt_b, m_pc_reg.m_q_w_curr.toRotationMatrix(),
          m_pc_reg.m_t_w_curr);
      temp_res.set_point_cloud(
          PCL_TOOLS::pcl_pts_to_eigen_pts<PT_DATA_TYPE, pcl_pt>(
              all_pt_temp.makeShared()));
      ...
      keyframe_a->save_to_file(save_path,
                               std::to_string(pair_idx).append("_a.json"));
      keyframe_b->save_to_file(save_path,
                               std::to_string(pair_idx).append("_b.json"));
      temp_res.save_to_file(save_path,
                            std::to_string(pair_idx).append("_c.json"));
