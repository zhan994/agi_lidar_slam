...
struct Eigen_pt_hasher {
  template <typename _T>
  std::size_t operator()(const _T& k) const {
    return ((std::hash<float>()(k(0)) ^ (std::hash<float>()(k(1)) << 1)) >> 1) ^
           (std::hash<float>()(k(2)) << 1);
  }
};
...
pcl::PointCloud<PointType> eigen_pt_to_pcl_pointcloud(
    const vector<T>& eigen_pt_vec) {
...
pcl::PointCloud<PointType> pointcloud_transfrom(
    pcl::PointCloud<PointType> pcl_pt_in, Eigen::Matrix<T, 3, 3> tranfrom_R,
    Eigen::Matrix<T, 3, 1> tranfrom_T) {
  ...
  for (size_t i = 0; i < pcl_pt_in.size(); i++) {
    auto eigen_pt =
        PCL_TOOLS::pcl_pt_to_eigen<T, PointType>(pcl_pt_in.points[i]);
    pcl_pc_res.points.push_back(PCL_TOOLS::eigen_to_pcl_pt<PointType>(
        tranfrom_R * eigen_pt + tranfrom_T));
  }
  return pcl_pc_res;
}

int save_PCLXYZI_to_txt(
    const std::string& filename,
    const pcl::PointCloud<pcl::PointXYZI>::ConstPtr pcl_pt_vec,
    int if_append = 0) {
  std::fstream fos;
  if (if_append > 0) {
  ...
  if (fos.is_open()) {
    for (size_t idx = 0; idx < pcl_pt_vec->size(); idx++) {
      fos << pcl_pt_vec->points[idx].x << "," << pcl_pt_vec->points[idx].y
          << "," << pcl_pt_vec->points[idx].z << ","
          << pcl_pt_vec->points[idx].intensity << std::endl;
    }
    fos.flush();
    fos.close();
    std::cout << "Save points to " << filename
              << " finish, points number =  " << pcl_pt_vec->size() << endl;
    return 0;
  } else {
    return -1;
  }
...
class PCL_point_cloud_to_pcd {
 public:
  ...
  void save_to_pcd_files(string file_name, T& cloud, int index = 0) {
    ...
    if (index == 0) {
      sprintf(tempchar, "%s/%s_%d.pcd", m_save_dir_name.c_str(),
              file_name.c_str(), m_save_files_index);
      m_save_files_index++;
    } else {
      sprintf(tempchar, "%s/%s_%d.pcd", m_save_dir_name.c_str(),
              file_name.c_str(), index);
...
template <typename T>
int load_from_pcd_file(const char* file_name,
                       pcl::PointCloud<T>& target_cloud) {
...
void rgb2hsv(const unsigned char& src_r, const unsigned char& src_g,
             const unsigned char& src_b, unsigned char& dst_h,
...
void hsv2rgb(const unsigned char& src_h, const unsigned char& src_s,
             const unsigned char& src_v, unsigned char& dst_r,
  ...
  // cout << "[" << ( int ) dst_r << " ," << ( int ) dst_g << " ," << ( int )
  // dst_b << " ]" << endl;
}

pcl::PointCloud<pcl::PointXYZRGBA> PCL_XYZI_to_RGBA(
    pcl::PointCloud<pcl::PointXYZI>& pt_in, float alpha = 0.1) {
  int size = pt_in.size();
  float min_val = 3e8;
  float max_val = -3e8;
...
template <typename T, typename PointType>
vector<Eigen::Matrix<T, 3, 1> > pcl_pts_to_eigen_pts(
  ...
  for (size_t i = 0; i < pc_size; i++) {
    // out_res[ i ] << input_cloud->points[ i ]x, input_cloud->points[ i ].y,
