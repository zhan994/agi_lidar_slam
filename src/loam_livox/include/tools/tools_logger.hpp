...
// #define FILE_LOGGER_VERSION_INFO "Add macro code block, enable turn off the
// printf quickly"

#define FILE_LOGGER_VERSION "V1.5"
#define FILE_LOGGER_VERSION_INFO                                          \
  "Add variable restore block, enable turn off/on screen prinf in local " \
  "block."
...
#define ADD_SCREEN_PRINTF_OUT_METHOD                                    \
  ...
  std::stringstream m_sceen_output_stringstream =                       \
      std::stringstream(m_sceen_output_string);                         \
...
#define ENABLE_SCREEN_PRINTF                             \
  Common_tools::Variable_restore_point<int> val_restore( \
...
class File_logger {
 public:
  ...
  File_logger(string _dir_name = string("/home/ziv/data/")) {
    ...
    m_map_file_os.insert(
        std::pair<string, std::ostream*>("screen", &std::cout));
  ...
  void init(std::string _file_name, std::string prefix_name = string("log"),
            int mode = std::ios::out) {
    std::ofstream* ofs = new std::ofstream();
    sprintf(m_temp_char, "%s/%s_%s", m_save_dir_name.c_str(),
            prefix_name.c_str(), _file_name.c_str());
    ...
    if (ofs->is_open()) {
      ...
      m_map_file_os.insert(
          std::pair<string, std::ostream*>(prefix_name, &std::cout));
    }
  };

  std::ostream* get_ostream(std::string prefix_name = string("log")) {
    if (m_if_verbose) return &m_temp_stringstream;
