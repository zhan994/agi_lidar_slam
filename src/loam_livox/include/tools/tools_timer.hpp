...
class Timer {
 public:
  typedef std::map<std::string,
      ...
      Map_string_timepoint;
  typedef std::map<std::string,
                   std::chrono::time_point<std::chrono::system_clock> >::
      iterator Map_string_timepoint_it;
 ...
 public:
  ...
  double toc(std::string str = std::string(" "), int if_retick = 1) {
    Map_string_timepoint_it it =
        find_timepoint(str.append(get_thread_id_str()));
  ...
  std::string toc_string(std::string str = std::string(" "),
                         int if_retick = 1) {
    sprintf(m_temp_char, "[Timer](%s): %s cost time = %.2f ms ",
            get_thread_id_str().c_str(), str.c_str(), toc(str, if_retick));
