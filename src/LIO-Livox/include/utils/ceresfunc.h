  ...
  bool Evaluate(double const* const* parameters, double* residuals,
    ...
    for (int i = 0;
      ...
      if (size == 6) {
        ...
        dx.segment<3>(idx + 3) = (Sophus::SO3d::exp(x.segment<3>(3)).inverse() *
      ...
      for (int i = 0;
        ...
        if (jacobians[i]) {
          ...
          jacobian.leftCols(size) =
              marginalization_info->linearized_jacobians.middleCols(idx, size);
        }
      }
    }
    return true;
...
struct Cost_NavState_PRV_Bias {
  Cost_NavState_PRV_Bias(IMUIntegrator& measure_, Eigen::Vector3d& GravityVec_,
  ...
  bool operator()(const T* pri_, const T* velobiasi_, const T* prj_,
    ...
    Eigen::Matrix<T, 3, 1> dbgi =
        velobiasi.template segment<3>(3) - imu_measure.GetBiasGyr().cast<T>();
    Eigen::Matrix<T, 3, 1> dbai =
        velobiasi.template segment<3>(6) - imu_measure.GetBiasAcc().cast<T>();
    ...
    Eigen::Matrix<T, 3, 1> rPij =
        ...
        (dPij +
         imu_measure.GetJacobian()
                 .block<3, 3>(IMUIntegrator::O_P, IMUIntegrator::O_BG)
                 .cast<T>() *
             dbgi +
         imu_measure.GetJacobian()
                 .block<3, 3>(IMUIntegrator::O_P, IMUIntegrator::O_BA)
                 .cast<T>() *
             dbai);

    Sophus::SO3<T> dR_dbg = Sophus::SO3<T>::exp(
        imu_measure.GetJacobian()
            .block<3, 3>(IMUIntegrator::O_R, IMUIntegrator::O_BG)
            .cast<T>() *
        dbgi);
    ...
    Eigen::Matrix<T, 3, 1> rVij =
        ...
        (dVij +
         imu_measure.GetJacobian()
                 .block<3, 3>(IMUIntegrator::O_V, IMUIntegrator::O_BG)
                 .cast<T>() *
             dbgi +
         imu_measure.GetJacobian()
                 .block<3, 3>(IMUIntegrator::O_V, IMUIntegrator::O_BA)
                 .cast<T>() *
             dbai);
  ...
  static ceres::CostFunction* Create(
      IMUIntegrator& measure_, Eigen::Vector3d& GravityVec_,
      Eigen::Matrix<double, 15, 15> sqrt_information_) {
    return (
        new ceres::AutoDiffCostFunction<Cost_NavState_PRV_Bias, 15, 6, 9, 6, 9>(
            new Cost_NavState_PRV_Bias(measure_, GravityVec_,
...
struct Cost_NavState_IMU_Line {
  Cost_NavState_IMU_Line(Eigen::Vector3d _p, Eigen::Vector3d _vtx1,
                         Eigen::Vector3d _vtx2, const Eigen::Matrix4d& Tbl,
  ...
  bool operator()(const T* PRi, T* residual) const {
    ...
    Eigen::Quaternion<T> q_wb =
        Sophus::SO3<T>::exp(pri_wb.template segment<3>(3)).unit_quaternion();
    ...
    T a012 = ceres::sqrt(((P_to_Map(0) - lpa(0)) * (P_to_Map(1) - lpb(1)) -
                         ...
                         ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) -
                          (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2))) *
                             ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) -
                              (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2))));
    T ld2 = a012 / T(l12);
    T _weight = T(1) - T(0.9) * ceres::abs(ld2) /
  ...
  static ceres::CostFunction* Create(
      ...
      const Eigen::Vector3d& last_point_b_, const Eigen::Matrix4d& Tbl,
      Eigen::Matrix<double, 1, 1> sqrt_information_) {
    return (new ceres::AutoDiffCostFunction<Cost_NavState_IMU_Line, 1, 6>(
        new Cost_NavState_IMU_Line(curr_point_, last_point_a_, last_point_b_,
                                   Tbl, std::move(sqrt_information_))));
  }
...
struct Cost_NavState_IMU_Plan {
  Cost_NavState_IMU_Plan(Eigen::Vector3d _p, double _pa, double _pb, double _pc,
                         double _pd,
  ...
  bool operator()(const T* PRi, T* residual) const {
    ...
    T pd2 =
        T(pa) * P_to_Map(0) + T(pb) * P_to_Map(1) + T(pc) * P_to_Map(2) + T(pd);
    T _weight = T(1) - T(0.9) * ceres::abs(pd2) /
                           ceres::sqrt(ceres::sqrt(P_to_Map(0) * P_to_Map(0) +
                                                   P_to_Map(1) * P_to_Map(1) +
                                                   P_to_Map(2) * P_to_Map(2)));
  ...
  static ceres::CostFunction* Create(
      const Eigen::Vector3d& curr_point_, const double& pa_, const double& pb_,
      const double& pc_, const double& pd_, const Eigen::Matrix4d& Tbl,
      Eigen::Matrix<double, 1, 1> sqrt_information_) {
    return (new ceres::AutoDiffCostFunction<Cost_NavState_IMU_Plan, 1, 6>(
        new Cost_NavState_IMU_Plan(curr_point_, pa_, pb_, pc_, pd_, Tbl,
                                   std::move(sqrt_information_))));
  }
...
struct Cost_NavState_IMU_Plan_Vec {
  ...
  bool operator()(const T* PRi, T* residual) const {
    ...
    T _weight = T(1) - T(0.9) * (P_to_Map - cp_proj).norm() /
                           ceres::sqrt(ceres::sqrt(P_to_Map(0) * P_to_Map(0) +
                                                   P_to_Map(1) * P_to_Map(1) +
                                                   P_to_Map(2) * P_to_Map(2)));
    eResiduals *= _weight;
  ...
  static ceres::CostFunction* Create(
    ...
    return (new ceres::AutoDiffCostFunction<Cost_NavState_IMU_Plan_Vec, 3, 6>(
        new Cost_NavState_IMU_Plan_Vec(curr_point_, p_proj_, Tbl,
                                       sqrt_information_)));
  }
...
struct Cost_NonFeature_ICP {
  Cost_NonFeature_ICP(Eigen::Vector3d _p, double _pa, double _pb, double _pc,
                      double _pd, const Eigen::Matrix4d& Tbl,
                      Eigen::Matrix<double, 1, 1> sqrt_information_)
      : point(std::move(_p)),
        pa(_pa),
        pb(_pb),
  ...
  static ceres::CostFunction* Create(
    ...
    return (new ceres::AutoDiffCostFunction<Cost_NonFeature_ICP, 1, 6>(
        new Cost_NonFeature_ICP(curr_point_, pa_, pb_, pc_, pd_, Tbl,
                                std::move(sqrt_information_))));
  }
...
struct Cost_Initialization_IMU {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  Cost_Initialization_IMU(IMUIntegrator& measure_, Eigen::Vector3d ri_,
  ...
  bool operator()(const T* rwg_, const T* vi_, const T* vj_, const T* ba_,
    ...
    Eigen::Matrix<T, 3, 1> rPij =
        RiT * (dP - Vi * dTij - SO3_Rwg * G_I * dT2 * T(0.5)) -
        (dPij +
         imu_measure.GetJacobian()
                 .block<3, 3>(IMUIntegrator::O_P, IMUIntegrator::O_BG)
                 .cast<T>() *
  ...
  static ceres::CostFunction* Create(
      IMUIntegrator& measure_, Eigen::Vector3d ri_, Eigen::Vector3d rj_,
      Eigen::Vector3d dp_, Eigen::Matrix<double, 9, 9> sqrt_information_) {
    return (new ceres::AutoDiffCostFunction<Cost_Initialization_IMU, 9, 3, 3, 3,
                                            3, 3>(new Cost_Initialization_IMU(
        measure_, ri_, rj_, dp_, std::move(sqrt_information_))));
  }
...
struct Cost_Initialization_Prior_bv {
  ...
  static ceres::CostFunction* Create(Eigen::Vector3d prior_,
    ...
    return (new ceres::AutoDiffCostFunction<Cost_Initialization_Prior_bv, 3, 3>(
        new Cost_Initialization_Prior_bv(prior_,
                                         std::move(sqrt_information_))));
  }
