...
int GetNeiborPCA_cor(SNeiborPCA_cor& npca,
  ...
  if (kdtree.radiusSearch(searchPoint, fSearchRadius, npca.neibors, k_dis) >
    ...
    for (int pid = 0; pid < subCloud->points.size();
         pid++)  //搜索半径内的地面点云 sy
    {
    ...
    npca.eigenValuesPCA(0) =
        npca.eigenValuesPCA(0) / (vsum + 0.000001);  //单位化 sy
...
int FilterGndForPos_cor(float* outPoints, float* inPoints, int inNum) {
  ...
  for (int pid = 0; pid < inNum; pid++) {
    if (outNum >= 60000) break;
    if (idtemp[pid] > 0 && idtemp[pid] < nx * ny) {
      imgMeanZ[idtemp[pid]] =
          float(imgSumZ[idtemp[pid]] / (imgNumZ[idtemp[pid]] + 0.0001));
      //最高点与均值高度差小于阈值；点数大于3；均值高度小于1
      if ((imgMaxZ[idtemp[pid]] - imgMeanZ[idtemp[pid]]) < THR &&
          imgNumZ[idtemp[pid]] > 3 &&
          imgMeanZ[idtemp[pid]] < 2) {  // imgMeanZ[idtemp[pid]]<0&&
...
int CalGndPos_cor(float* gnd, float* fPoints, int pointNum,
                  float fSearchRadius) {
  // 初始化gnd
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if ((nNum < 1000) && (pLabel[pid] == 0)) {
      ...
      if (GetNeiborPCA_cor(npca, cloud, kdtree, searchPoint, fSearchRadius) >
          0) {
        for (int ii = 0; ii < npca.neibors.size(); ii++) {
          pLabel[npca.neibors[ii]] = 1;
        }

        if (npca.eigenValuesPCA[1] / (npca.eigenValuesPCA[0] + 0.00001) >
            5000) {  //指的是主方向与次方向差异较大。即这一小块接近平面 sy
...
int CorrectPoints_cor(float* fPoints, int pointNum, float* gndPos) {
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    ...
    tmp[2] = RTM[2] * fPoints[pid * 4] + RTM[5] * fPoints[pid * 4 + 1] +
             RTM[8] * fPoints[pid * 4 + 2] - gndHeight;
...
int GetGndPos(float* pos, float* fPoints, int pointNum) {
  ...
  int gndnum =
      CalGndPos_cor(tmpPos, fPoints3, pnum3,
                    1.0);  //用法向量判断，获取到法向量 & 地面搜索点，放到tmppos
  if (gnd_pos[5] == 0) {
    memcpy(gnd_pos, tmpPos, sizeof(tmpPos));
  } else {
    if (frame_count < frame_lenth_threshold && tmpPos[5] != 0) {
      if (gndnum > 0 && abs(gnd_pos[0] - tmpPos[0]) < 0.1 &&
          abs(gnd_pos[1] - tmpPos[1]) < 0.1) {  //更新法向量
        for (int i = 0; i < 6; i++) {
