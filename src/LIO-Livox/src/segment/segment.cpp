...
PCSeg::PCSeg() {
  this->posFlag = 0;
  this->pVImg = (unsigned char*)calloc(
      DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY * DN_SAMPLE_IMG_NZ,
...
int PCSeg::DoSeg(int* pLabel1, float* fPoints1, int pointNum) {
  ...
  memset(pVImg, 0,
         sizeof(unsigned char) * DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY *
             DN_SAMPLE_IMG_NZ);  // 600*200*30

  for (int pid = 0; pid < pointNum; pid++) {
    int ix = (fPoints1[pid * 4] + DN_SAMPLE_IMG_OFFX) /
             DN_SAMPLE_IMG_DX;  // 0-240m -> -40-190m
    int iy = (fPoints1[pid * 4 + 1] + DN_SAMPLE_IMG_OFFY) /
             DN_SAMPLE_IMG_DY;  //-40-40m
    int iz = (fPoints1[pid * 4 + 2] + DN_SAMPLE_IMG_OFFZ) /
             DN_SAMPLE_IMG_DZ;  //认为地面为-1.8？ -2.5~17.5

    idtrans1[pid] = -1;
    if ((ix >= 0) && (ix < DN_SAMPLE_IMG_NX) && (iy >= 0) &&
        (iy < DN_SAMPLE_IMG_NY) && (iz >= 0) &&
        (iz < DN_SAMPLE_IMG_NZ))  // DN_SAMPLE_IMG_OFFX = 0 因此只保留前半块
    {
      idtrans1[pid] = iz * DN_SAMPLE_IMG_NX * DN_SAMPLE_IMG_NY +
                      iy * DN_SAMPLE_IMG_NX + ix;  //记录这个点对应的索引
      if (pVImg[idtrans1[pid]] ==
          0)  //没有访问过，肯定栅格内会有重复的，所以fPoints2只取第一个
      {
  ...
  tmpPos[4] = 0;
  tmpPos[5] = -2.04;
  GetGndPos(tmpPos, fPoints2,
            pntNum);  // tempPos是更新后的地面搜索点 & 平均法向量 ys
  ...
  if (this->corPoints != NULL) free(this->corPoints);
...
int PCSeg::EncodeFeatures(float* pFeas) {
  ...
  // pFeas:(1+256*6)*3
  // 0        3 ~ 3+356*3 = 257*3        275*3 ~ 257*3+256*3             3+256*6
  // ~ 3+256*6 + 256*4*3+12+1 number    center 每个障碍物的点数、类别、zmax OBB
  // 12个点 clusterNum
  ...
  for (int ii = 0; ii < 256; ii++) {
    for (int jj = 0; jj < 4; jj++) {
      pFeas[257 * 3 + 256 * 3 + (ii * 4 + jj) * 3] =
          this->pOBBs[ii * 8 + jj * 2];
      pFeas[257 * 3 + 256 * 3 + (ii * 4 + jj) * 3 + 1] =
          this->pOBBs[ii * 8 + jj * 2 + 1];
    }
  }
}
int FilterGndForPos(float* outPoints, float* inPoints, int inNum) {
  ...
  for (int pid = 0; pid < inNum; pid++) {
    idtemp[pid] = -1;
    if ((inPoints[pid * 4] < x_len) && (inPoints[pid * 4 + 1] > -y_len) &&
        (inPoints[pid * 4 + 1] < y_len)) {
      ...
      if (inPoints[pid * 4 + 2] <
          imgMinZ[idx + idy * nx])  //寻找最小点，感觉不对。最小点有误差。
      {
  ...
  for (int pid = 0; pid < inNum; pid++) {
    if (idtemp[pid] > 0) {
      imgMeanZ[idtemp[pid]] =
          float(imgSumZ[idtemp[pid]] / imgNumZ[idtemp[pid]]);
      if ((imgMaxZ[idtemp[pid]] - imgMinZ[idtemp[pid]]) < THR &&
          imgMeanZ[idtemp[pid]] < -1.5 && imgNumZ[idtemp[pid]] > 3) {
...
int CalNomarls(float* nvects, float* fPoints, int pointNum,
               float fSearchRadius) {
  // 转换点云到pcl的格式
  ...
  unsigned char* pLabel =
      (unsigned char*)calloc(pointNum, sizeof(unsigned char));
  for (int pid = 0; pid < pointNum; pid++) {
    if ((nNum < 10) && (pLabel[pid] == 0)) {
      ...
      if (GetNeiborPCA(npca, cloud, kdtree, searchPoint, fSearchRadius) > 0) {
        ...
        if ((npca.eigenValuesPCA[1] > 0.4) && (npca.eigenValuesPCA[0] < 0.01)) {
          if ((npca.eigenVectorsPCA(2, 0) > 0.9) |
              (npca.eigenVectorsPCA(2, 0) < -0.9)) {
...
int CalGndPos(float* gnd, float* fPoints, int pointNum, float fSearchRadius) {
  ...
  for (int cur = 0; cur < pointNum; cur++) {
    if (fPoints[cur * 4 + 2] > height_mean + 0.5 ||
        fPoints[cur * 4 + 2] < height_mean - 0.5) {
      pointNum--;
    }
  }
  ...
  for (int pid = 0; pid < cloud->points.size(); pid++) {
    if (fPoints[pid * 4 + 2] <= height_mean + 0.5 &&
        fPoints[pid * 4 + 2] >= height_mean - 0.5) {
      cloud->points[pid].x = fPoints[pid * 4];
      cloud->points[pid].y = fPoints[pid * 4 + 1];
      cloud->points[pid].z = fPoints[pid * 4 + 2];
    }
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if ((nNum < 1000) && (pLabel[pid] == 0)) {
      ...
      if (GetNeiborPCA(npca, cloud, kdtree, searchPoint, fSearchRadius) > 0) {
        ...
        // 地面向量筛选
        if ((npca.eigenValuesPCA[1] > 0.2) && (npca.eigenValuesPCA[0] < 0.1) &&
            searchPoint.x > 29.9 && searchPoint.x < 40)  //&&searchPoint.x>19.9
        {
          if ((npca.eigenVectorsPCA(2, 0) > 0.9) |
              (npca.eigenVectorsPCA(2, 0) < -0.9)) {
            if (npca.eigenVectorsPCA(2, 0) > 0) {
...
int GetNeiborPCA(SNeiborPCA& npca, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                 pcl::KdTreeFLANN<pcl::PointXYZ> kdtree,
                 pcl::PointXYZ searchPoint, float fSearchRadius) {
  std::vector<float> k_dis;
  pcl::PointCloud<pcl::PointXYZ>::Ptr subCloud(
      new pcl::PointCloud<pcl::PointXYZ>);

  if (kdtree.radiusSearch(searchPoint, fSearchRadius, npca.neibors, k_dis) >
      5) {
    ...
    float vsum = npca.eigenValuesPCA(0) + npca.eigenValuesPCA(1) +
                 npca.eigenValuesPCA(2);
...
int GetRTMatrix(float* RTM, float* v0, float* v1)  // v0 gndpos v1:001垂直向上
{
  ...
  //
  float v2
      [3];  // 叉乘的向量代表了v0旋转到v1的旋转轴，因为叉乘结果垂直于v0、v1构成的平面
  ...
  cosAng =
      v0[0] * v1[0] + v0[1] * v1[1] + v0[2] * v1[2];  // a.b = |a||b|cos theta
...
int PCSeg::CorrectPoints(float* fPoints, int pointNum, float* gndPos) {
  ...
  GetRTMatrix(
      RTM, gndPos,
      znorm);  // RTM 由当前地面法向量gnd，将平面转到以(0,0,1)为法向量的平面 sy

  gndHeight = RTM[2] * gndPos[3] + RTM[5] * gndPos[4] + RTM[8] * gndPos[5];

  for (int pid = 0; pid < pointNum; pid++) {
    tmp[0] = RTM[0] * fPoints[pid * 4] + RTM[3] * fPoints[pid * 4 + 1] +
             RTM[6] * fPoints[pid * 4 + 2];
    tmp[1] = RTM[1] * fPoints[pid * 4] + RTM[4] * fPoints[pid * 4 + 1] +
             RTM[7] * fPoints[pid * 4 + 2];
    tmp[2] = RTM[2] * fPoints[pid * 4] + RTM[5] * fPoints[pid * 4 + 1] +
             RTM[8] * fPoints[pid * 4 + 2] - gndHeight;  //将地面高度变换到0 sy
...
int SegBG(int* pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
          pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree, float fSearchRadius) {
  ...
  while (seeds.size() > 0) {
    ...
    for (int ii = 0; ii < k_inds.size(); ii++) {
      if (pLabel[k_inds[ii]] == 0) {
        pLabel[k_inds[ii]] = 1;
        if (cloud->points[k_inds[ii]].z >
            0.2)  //地面60cm以下不参与背景分割，以防止错误的地面点导致过度分割
        {
...
SClusterFeature FindACluster(int* pLabel, int seedId, int labelId,
                             ...
                             pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree,
                             float fSearchRadius, float thrHeight) {
  // 初始化种子
  std::vector<int> seeds;
  ...
  // 区域增长
  while (seeds.size() >
         0)  //实际上是种子点找了下k近邻，然后k近邻再纳入种子点 sy
  {
    int sid = seeds[seeds.size() - 1];
    seeds.pop_back();
...
int SegObjects(int* pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
               pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree, float fSearchRadius) {
  int pnum = cloud->points.size();
  ...
  for (int pid = 0; pid < pnum; pid++) {
    if (pLabel[pid] == 0) {
      ...
      {
        SClusterFeature cf =
            FindACluster(pLabel, pid, labelId, cloud, kdtree, fSearchRadius, 0);
        int isBg = 0;
...
int CompleteObjects(int* pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                    pcl::KdTreeFLANN<pcl::PointXYZ>& kdtree,
                    float fSearchRadius) {
  int pnum = cloud->points.size();
...
int CalFreeRegion(float* pFreeDis, float* fPoints, int* pLabel, int pointNum) {
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if ((pLabel[pid] == 1) && (fPoints[pid * 4 + 2] < 4.5)) {
      dis = fPoints[pid * 4] * fPoints[pid * 4] +
            fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1];
      thetaId = ((atan2f(fPoints[pid * 4 + 1], fPoints[pid * 4]) + FREE_PI) /
                 FREE_DELTA_ANG);
...
int FreeSeg(float* fPoints, int* pLabel, int pointNum) {
  ...
  if (pFreeDis != NULL) free(pFreeDis);
}
...
int GndSeg(int* pLabel, float* fPoints, int pointNum, float fSearchRadius) {
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if (tmpLabel1[pid] >= 0) {
      if (pGndImg1[tmpLabel1[pid]] + 0.5 >
          fPoints[pid * 4 +
                  2])  // 相对高度限制
                       // 即pid所在点对应的格子里面高度不超过0.5，标记为1
      {
        {
          pLabel[pid] = 1;
          pnum++;
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if (pLabel[pid] == 1) {
      if (fPoints[pid * 4 + 2] >
          1)  // for all cases
              // 即使这个点所在格子高度差小于0.5，但绝对高度大于1，那也不行
      {
        pLabel[pid] = 0;
      } else if (fPoints[pid * 4] * fPoints[pid * 4] +
                     fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1] <
                 225)  // 10m内，强制要求高度小于0.5
      {
      ...
      if (fPoints[pid * 4] * fPoints[pid * 4] +
              fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1] <
          400)  // 20m内，0.2以下强制设为地面
      {
...
SClusterFeature CalOBB(float* fPoints, int pointNum) {
  ...
  for (int ii = 0; ii < 180; ii++) {
    ...
    for (int pid = 0; pid < pointNum; pid++) {
      float val = fPoints[pid * 4] * coef_as[ii] +
                  fPoints[pid * 4 + 1] * coef_bs[ii];  // x*cos + y*sin
                                                       // 这是把每个点旋转一下，然后求旋转后点云的xy最大最小值
                                                       // sy
      if (a_min > val) a_min = val;
      if (a_max < val) a_max = val;
      rxy[pid * 2] = val;

      val = fPoints[pid * 4 + 1] * coef_as[ii] - fPoints[pid * 4] * coef_bs[ii];
      if (b_min > val) b_min = val;
      if (b_max < val) b_max = val;
      rxy[pid * 2 + 1] = val;
    }
    ...
    if (weights > area)  //(b_max-b_min)*(a_max-a_min)<area)
    {
      ...
      cf.center[0] =
          (a_max + a_min) / 2 * coef_as[ii] - (b_max + b_min) / 2 * coef_bs[ii];
      cf.center[1] =
          (a_max + a_min) / 2 * coef_bs[ii] + (b_max + b_min) / 2 * coef_as[ii];
      cf.center[2] = 0;
  ...
  for (int pid = 0; pid < pointNum; pid++) {
    if (fPoints[pid * 4 + 2] > z_max) z_max = fPoints[pid * 4 + 2];
    if (fPoints[pid * 4 + 2] < z_min) z_min = fPoints[pid * 4 + 2];
  }
  ...
  } else if (dx < 0.5 && dy < 0.5)  // too small
  {
    cf.cls = 1;
             ...
             (dx > 6 || dy > 6 || dx / dy > 5 || dy / dx > 5))  // too small
  {
    // cf.cls=1;
