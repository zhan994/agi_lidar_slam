...
/**
 * \brief // api: 判断点的时间先后顺序(注意curvature中存储的是时间戳)
 *
 * \param x
 * \param y
 * \return true
...
class ImuProcess {
 public:
  ...
  /**
   * \brief // api: ImuProcess构造
   *
   */
  ImuProcess();

  /**
   * \brief // api: ImuProcess析构
   *
   */
  ~ImuProcess();
   ...
   * \param acc_bias
   */
  void set_param(const V3D& transl, const M3D& rot, const V3D& gyr,
                 const V3D& acc, const V3D& gyr_bias, const V3D& acc_bias);
   ...
   * \param meas 打包好的数据
   * \param kf_state 状态量
   * \param pcl_un_ 去畸变的点云
   */
  void Process(const MeasureGroup& meas, esekfom::esekf& kf_state,
...
void ImuProcess::set_param(const V3D& transl, const M3D& rot, const V3D& gyr,
                           const V3D& acc, const V3D& gyr_bias,
                           const V3D& acc_bias) {
...
void ImuProcess::IMU_init(const MeasureGroup& meas, esekfom::esekf& kf_state,
  ...
  if (b_first_frame_) {
    ...
    const auto& imu_acc =
        meas.imu.front()->linear_acceleration;  // IMU初始时刻的加速度
    const auto& gyr_acc =
        meas.imu.front()->angular_velocity;  // IMU初始时刻的角速度
    mean_acc << imu_acc.x, imu_acc.y, imu_acc.z;  //第一帧加速度值作为初始化均值
    mean_gyr << gyr_acc.x, gyr_acc.y, gyr_acc.z;  //第一帧角速度值作为初始化均值
    first_lidar_time =
        meas.lidar_beg_time;  //将当前IMU帧对应的lidar起始时间 作为初始时间
  }

  // step: 2 根据所有IMU数据，计算平均值和方差
  ...
  //将初始化后的状态传入esekfom.hpp中的x_
  kf_state.change_x(init_state);

  // step:4 在esekfom.hpp获得P_的协方差矩阵并更新
...
void ImuProcess::UndistortPcl(const MeasureGroup& meas,
  ...
  // step: 1 将上一帧最后尾部的imu添加到当前帧头部的imu
  // 取出当前帧的IMU队列,将上一帧最后尾部的imu添加到当前帧头部的imu
  auto v_imu = meas.imu;
  ...
  // step: 2 根据点云中每个点的时间戳对点云进行重排序
  pcl_out = *(meas.lidar);
  // curvature中存放了时间戳（在preprocess.cpp中）
  sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);

  // step: 3 获取上一次KF估计的后验状态作为本次IMU预测的初始状态
  state_ikfom imu_state = kf_state.get_x();
  IMUpose.clear();
  // 初始状态加入IMUpose中,有时间间隔，上一帧加速度，上一帧角速度，上一帧速度，上一帧位置，上一帧旋转矩阵
  IMUpose.push_back(set_pose6d(0.0, acc_s_last, angvel_last, imu_state.vel,
                               imu_state.pos, imu_state.rot.matrix()));

  // step: 4 前向传播
  // 平均角速度，平均加速度，imu加速度，imu速度，imu位置
  V3D angvel_avr, acc_avr, acc_imu, vel_imu, pos_imu;
  M3D R_imu;  // IMU旋转矩阵 消除运动失真的时候用
  double dt = 0;
  input_ikfom in;
  // step: 4.1 遍历本次估计的所有IMU测量并且进行积分，离散中值法 前向传播
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    auto&& head = *(it_imu);      //拿到当前帧的imu数据
    auto&& tail = *(it_imu + 1);  //拿到下一帧的imu数据
    // step: 4.1.1 判断时间先后顺序，下一帧时间戳是否小于上一帧结束时间戳
    // 不符合直接continue
    ...
    // step: 4.1.4 两帧IMU的中值作为输入in  用于前向传播
    in.acc = acc_avr;
    in.gyro = angvel_avr;
    ...
    // step: 4.1.6 IMU前向传播，每次传播的时间间隔为dt
    kf_state.predict(dt, Q, in);

    // step: 4.1.7 更新IMU状态为积分后的状态
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
    ...
    //此时从IMUpose的末尾开始循环，也就是从时间最大处开始，因此只需要判断
    //点云时间需>IMU head时刻  即可   不需要判断 点云时间<IMU tail
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
      dt = it_pcl->curvature / double(1000) -
           head->offset_time;  //点到IMU开始时刻的时间间隔
...
void ImuProcess::Process(const MeasureGroup& meas, esekfom::esekf& kf_state,
  ...
  // step: 2 The very first lidar frame 初始化
  if (imu_need_init_) {
    // step: 2.1 IMU_init
    IMU_init(meas, kf_state, init_iter_num);
    imu_need_init_ = true;

    // step: 2.2 获取初始化后的状态变量
    last_imu_ = meas.imu.back();
    state_ikfom imu_state = kf_state.get_x();
